...
const pricing = [
  { tier:'Starter', price:'$149', per:'/month', featured:false, feats:['1 Active Project','Up to 25 Contractors','AI Recommendations','Automated SMS (500/mo)','Owner Reports','Email Support'] },
  { tier:'Pro', price:'$349', per:'/month', featured:true, feats:['5 Active Projects','Unlimited Contractors','Full AI Scheduling Agent','Automated SMS (3,000/mo)','Gantt + Bidding System','Daily Photo Log','Priority Support'] },
  { tier:'Enterprise', price:'Custom', per:'', featured:false, feats:['Unlimited Projects','Unlimited Contractors','White-label Option','Custom AI Training','API Access','Dedicated Account Manager','SLA Guarantee'] },
]
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content="The AI-powered construction management platform that automates scheduling, contractor coordination, bidding, and owner reporting." />
        <meta name="viewport" content="width=device-width, initial-scale=1" />
        <link rel="icon" href="/favicon.ico" />
      </Head>

      <nav style={s.nav}>
        <div style={s.logo}><div style={s.logoBox}>🏗</div>AI GENERAL CONTRACTOR</div>
        ...
        <div style={s.navCta}>
          <Link href="/dashboard"><button style={s.btnOutline}>View Demo</button></Link>
          <Link href="/dashboard"><button style={s.btnPrimary}>Start Free Trial</button></Link>
        </div>
      </nav>
      ...
      <div style={s.hero}>
        ...
        <div style={{ animation:'fadeUp 0.6s 0.1s ease both' }}>
          <h1 style={s.heroTitle}>YOUR JOBSITE.<br/><span style={s.heroTitleAccent}>RUN BY AI.</span></h1>
        </div>
        <div style={{ animation:'fadeUp 0.6s 0.2s ease both' }}>
          <p style={s.heroSub}>Stop spending your days on phone calls, spreadsheets, and text messages. AI General Contractor automates every coordination task — so you can build more and manage less.</p>
        </div>
        <div style={{ ...s.heroCtas, animation:'fadeUp 0.6s 0.3s ease both' }}>
          <Link href="/dashboard"><button style={{ ...s.btnPrimary, ...s.btnLg }}>🚀 Launch Free Demo</button></Link>
          <Link href="/dashboard"><button style={{ ...s.btnOutline, ...s.btnLg }}>See Full Dashboard →</button></Link>
        </div>
        <div style={{ ...s.heroStats, animation:'fadeUp 0.6s 0.4s ease both' }}>
          {[['20+','Hours saved per week'],['94%','Subcontractor response rate'],['$11k','Avg savings per bid cycle'],['3 min','Daily owner report time']].map(([v,l])=>(
            <div key={l} style={s.heroStat}><span style={s.heroStatVal}>{v}</span><div style={s.heroStatLbl}>{l}</div></div>
          ))}
        </div>
      </div>
      ...
      <div style={{ padding:'0 40px 80px', maxWidth:1200, margin:'0 auto' }}>
        <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:16, padding:4, boxShadow:'0 40px 80px rgba(0,0,0,0.6)' }}>
          <div style={{ background:'var(--surface2)', borderRadius:12, padding:'12px 16px', display:'flex', alignItems:'center', gap:8, borderBottom:'1px solid var(--border)' }}>
            {['#ff6b6b','var(--warn)','var(--ok)'].map((c,i)=><div key={i} style={{ width:10, height:10, borderRadius:'50%', background:c }} />)}
            <div style={{ flex:1, background:'var(--bg)', borderRadius:6, padding:'4px 12px', fontSize:11, color:'var(--muted)', fontFamily:'var(--font-mono)', textAlign:'center' }}>aigeneralcontractor.vercel.app/dashboard</div>
          </div>
          <Link href="/dashboard">
            <div style={{ padding:20, cursor:'pointer' }}>
              <div style={{ display:'grid', gridTemplateColumns:'180px 1fr 200px', gap:12, height:280 }}>
                <div style={{ background:'var(--bg)', borderRadius:10, padding:12, display:'flex', flexDirection:'column', gap:6 }}>
                  {['Monroe Apts Ph.1','Oak Ridge Retail','Elm St. Townhomes'].map((p,i)=>(
                    <div key={p} style={{ padding:'8px 10px', borderRadius:6, background:i===0?'rgba(232,197,71,0.1)':'transparent', border:i===0?'1px solid rgba(232,197,71,0.2)':'1px solid transparent', fontSize:11, color:i===0?'var(--accent)':'var(--muted)', display:'flex', alignItems:'center', gap:6 }}>
                      <div style={{ width:6, height:6, borderRadius:'50%', background:i===0?'var(--ok)':i===1?'var(--warn)':'var(--muted)' }} />{p}
                    </div>
                  ))}
                ...
                <div style={{ background:'var(--bg)', border:'1px solid var(--border)', borderRadius:10, padding:12, overflow:'hidden' }}>
                  ...
                  {[{p:'HIGH',t:'Accelerate Framing',c:'var(--danger)'},{p:'HIGH',t:'Award Roofing Contract',c:'var(--danger)'},{p:'MED',t:'Follow Up: Lumber',c:'var(--warn)'},{p:'LOW',t:'Send Owner Update',c:'var(--ok)'}].map(r=>(
                  ...
                  <div style={{ marginTop:10, padding:8, background:'linear-gradient(135deg,rgba(232,197,71,0.1),rgba(78,205,196,0.1))', border:'1px solid rgba(232,197,71,0.2)', borderRadius:6, fontSize:11, color:'var(--accent)', textAlign:'center', fontWeight:700 }}>⚡ RUN AI CHECK</div>
                </div>
              </div>
              <div style={{ textAlign:'center', marginTop:12, fontSize:12, color:'var(--accent)', fontWeight:600 }}>Click to open live dashboard →</div>
            </div>
          </Link>
        </div>
      </div>
      ...
      <div style={{ ...s.section, borderTop:'1px solid var(--border)' }}>
        ...
        <div style={s.featGrid}>
          {features.map((f,i)=>(
            <div key={f.title} style={{ ...s.featCard, ...(hoveredFeat===i?{borderColor:'rgba(232,197,71,0.3)',transform:'translateY(-4px)',boxShadow:'0 20px 40px rgba(0,0,0,0.4)'}:{}) }} onMouseEnter={()=>setHoveredFeat(i)} onMouseLeave={()=>setHoveredFeat(null)}>
          ...
          {testimonials.map(t=>(
            <div key={t.name} style={s.testCard}>
              ...
              <div style={s.testAuthor}>
                ...
                <div><div style={s.testName}>{t.name}</div><div style={s.testRole}>{t.role}</div></div>
              </div>
            </div>
          ))}
        </div>
      ...
      <div style={{ ...s.section, borderTop:'1px solid var(--border)' }}>
        <div style={{ textAlign:'center', marginBottom:0 }}>
          ...
          <p style={{ ...s.sectionSub, margin:'0 auto' }}>A full-time superintendent costs $90k/yr. AI GC costs less than one day of either.</p>
        </div>
        <div style={s.pricingGrid}>
          {pricing.map(p=>(
            <div key={p.tier} style={p.featured?s.priceCardFeatured:s.priceCard}>
              ...
              <ul style={s.priceFeats}>{p.feats.map(f=><li key={f} style={s.priceFeat}><span style={s.priceFeatCheck}>✓</span>{f}</li>)}</ul>
              <Link href="/dashboard">
      ...
      <div style={s.ctaBanner}>
        ...
        <div style={{ display:'flex', gap:12, justifyContent:'center', flexWrap:'wrap' }}>
          <Link href="/dashboard"><button style={{ ...s.btnPrimary, ...s.btnLg }}>🚀 Open Live Dashboard</button></Link>
          <Link href="/dashboard"><button style={{ ...s.btnOutline, ...s.btnLg }}>Schedule a Demo</button></Link>
        </div>
      </div>

      <footer style={s.footer}>
        <div style={s.footerLogo}>🏗 AI GENERAL CONTRACTOR</div>
        <div style={s.footerLinks}><span>Privacy</span><span>Terms</span><span>Contact</span><span>Docs</span></div>
