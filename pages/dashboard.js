...
const SCHEDULE = [
  { task:'Site Prep',          trade:'GC',          pct:100, status:'done' },
  { task:'Foundation',         trade:'Concrete',    pct:100, status:'done' },
  { task:'Framing – Unit 1',   trade:'Carpenter',   pct:100, status:'done' },
  { task:'Framing – Unit 2',   trade:'Carpenter',   pct:75,  status:'on' },
  { task:'Framing – Unit 3B ⚠',trade:'Carpenter',   pct:40,  status:'delay' },
  { task:'MEP Rough-In',       trade:'MEP',         pct:15,  status:'waiting' },
  { task:'Electrical Rough',   trade:'Electrician', pct:5,   status:'waiting' },
  { task:'Roofing',            trade:'Roofer',      pct:0,   status:'waiting' },
  { task:'Insulation',         trade:'Insulation',  pct:0,   status:'pending' },
  { task:'Drywall',            trade:'Drywaller',   pct:0,   status:'pending' },
  { task:'Finish Carpentry',   trade:'Carpenter',   pct:0,   status:'pending' },
  { task:'Punch & CO',         trade:'GC',          pct:0,   status:'pending' },
]
...
const SCHEDULE_COLORS = { done:'var(--accent2)', on:'var(--ok)', delay:'var(--danger)', waiting:'var(--warn)', pending:'var(--muted)' }

const DEFAULT_AI_RECS = [
...
export default function Dashboard() {
  ...
  const [aiRecs, setAiRecs]     = useState(DEFAULT_AI_RECS)
  ...
  async function runAI() {
    setAiRunning(true); setAiDone(false); setAiMessage('')
    try {
      const res = await fetch('/api/agent', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ schedule: SCHEDULE, logText }),
      })
      const data = await res.json()
      if (data.recommendations?.length > 0) {
        setAiRecs(data.recommendations)
        setDismissed([]); setActioned([])
        setAiMessage(`✓ Analysis complete. ${data.recommendations.length} live recommendations.`)
      } else {
        setAiMessage('✓ Analysis complete.')
      }
    } catch {
      setAiMessage('⚠ AI unavailable — showing cached recommendations.')
    } finally {
      setAiRunning(false); setAiDone(true)
    }
  }

  function sendSMS() { setSmsSent(true); setTimeout(() => setSmsSent(false), 4000) }
  function loadTemplate(key) {
    const name = smsTo.split(' ')[0] || 'Team'
    setSmsBody(SMS_TEMPLATES[key].replace('[Name]', name))
  }
  function composeTo(name, company, phone) {
    setSmsTo(`${name} – ${company}`); setSmsPhone(phone)
  ...
  return (
    <>
      ...
      <style>{`
        ...
        input,textarea,select{background:var(--bg);border:1px solid var(--border);border-radius:6px;padding:8px;color:var(--text);font-family:inherit;}
      ...
      <div style={{display:'grid',gridTemplateColumns:'220px 1fr 280px',height:'calc(100vh - 56px)',overflow:'hidden'}}>
        ...
        <div style={{background:'var(--surface)',borderRight:'1px solid var(--border)',padding:'20px 12px',overflowY:'auto',display:'flex',flexDirection:'column',gap:6}}>
          ...
          {[{name:'Monroe Apts Ph.1',color:'var(--ok)'},{name:'Oak Ridge Retail',color:'var(--warn)'},{name:'Elm St. Townhomes',color:'var(--muted)'}].map((p,i)=>(
            <button key={p.name} className={`sidebar-item${i===0?' active':''}`}>
              <div style={{width:8,height:8,borderRadius:'50%',background:p.color,flexShrink:0}}/><span>{p.name}</span>
            </button>
          ))}
          ...
          {[{icon:'📊',label:'Daily Report',v:'reports'},{icon:'💬',label:'Send SMS',v:'contractors'},{icon:'📋',label:'Bid Requests',v:'schedule'},{icon:'👷',label:'Contractors',v:'contractors'}].map(item=>(
            <button key={item.label} className="sidebar-item" onClick={()=>setView(item.v)}><span>{item.icon}</span><span>{item.label}</span></button>
          ))}
          <div style={{marginTop:'auto',padding:'12px 8px',borderTop:'1px solid var(--border)'}}>
            <div style={{fontSize:10,color:'var(--muted)',fontFamily:'var(--font-mono)'}}>v1.0.0</div>
            <div style={{fontSize:11,color:'var(--ok)',marginTop:4}}>⚡ AI Online</div>
          </div>
        </div>
        ...
        <div style={{overflowY:'auto',padding:24}}>
          ...
          <div className={`view${view==='dashboard'?' active':''}`}>
            <div style={{background:'linear-gradient(135deg,#141718,#1c1f20)',border:'1px solid var(--border)',borderRadius:12,padding:24,position:'relative',overflow:'hidden'}}>
              ...
              <div style={{marginTop:20}}>
                ...
                <div style={{background:'var(--bg)',borderRadius:100,height:10,overflow:'hidden'}}>
                  <div style={{width:'34%',height:'100%',borderRadius:100,background:'linear-gradient(90deg,var(--accent),var(--accent2))'}}/>
                </div>
                <div style={{display:'flex',justifyContent:'space-between',marginTop:6,fontSize:11,color:'var(--muted)'}}>
                  <span>Day 107 of 198</span><span style={{color:'var(--warn)'}}>⚠ 8 days behind schedule</span>
                </div>
              </div>
            ...
            <div className="card">
              ...
              <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:10}}>
                {[{bg:'linear-gradient(135deg,#1c2a1c,#2a3a2a)',icon:'🏗',label:'Unit 2A – Framing',tag:'TODAY 7:42 AM'},{bg:'linear-gradient(135deg,#1c1c2a,#2a2a3a)',icon:'🧱',label:'North Wall – Block Work',tag:'TODAY 9:15 AM'},{bg:'linear-gradient(135deg,#2a1c1c,#3a2a2a)',icon:'🚛',label:'Material Delivery – Lumber',tag:'YST 3:20 PM'},{bg:'linear-gradient(135deg,#1c2a2a,#2a3a3a)',icon:'🔧',label:'Plumbing Rough-In B1',tag:'YST 10:05 AM'},{bg:'linear-gradient(135deg,#2a2a1c,#3a3a2a)',icon:'⚡',label:'Electrical Panel Install',tag:'MON 2:30 PM'},{bg:'linear-gradient(135deg,#1c1c1c,#2a2a2a)',icon:'📡',label:'Connect Live Camera',tag:'+ ADD FEED'}].map(p=>(
              ...
              {SCHEDULE.slice(0,6).map(row=>(
                <div key={row.task} className="gantt-row">
                  ...
                  <div style={{background:'var(--bg)',borderRadius:4,height:18,overflow:'hidden'}}>
                    <div style={{height:'100%',borderRadius:4,background:SCHEDULE_COLORS[row.status]||'var(--muted)',width:`${Math.max(row.pct,row.pct>0?3:0)}%`}}/>
                  </div>
            ...
            <div className="card">
              ...
              {[{to:"→ Mike's Electric (Mike Johnson)",time:'TODAY 6:00 AM',msg:"Hi Mike, please confirm your electrical crew will arrive tomorrow at 7 AM for Monroe Apartments Phase 1. Reply CONFIRM.",tags:[{label:'✓ CONFIRMED',color:'var(--ok)',bg:'rgba(105,219,124,.1)'},{label:'SMS SENT',color:'var(--accent2)',bg:'rgba(78,205,196,.1)'}]},{to:'→ Johnson Framing (Dave Johnson)',time:'YST 4:30 PM',msg:"Hi Dave, framing on Unit 3B is delayed. Please provide an updated completion estimate.",tags:[{label:'⏳ AWAITING REPLY',color:'var(--warn)',bg:'rgba(255,169,77,.1)'},{label:'SMS SENT',color:'var(--accent2)',bg:'rgba(78,205,196,.1)'}]},{to:'→ 4 Roofing Contractors (Bid Request)',time:'YST 9:00 AM',msg:"Hi, we are requesting bids for roofing at Monroe Apartments. Est. 8,400 sqft TPO. Start: May 12. Reply BID.",tags:[{label:'4 SENT',color:'var(--accent2)',bg:'rgba(78,205,196,.1)'},{label:'2 RESPONSES',color:'var(--accent)',bg:'rgba(232,197,71,.1)'}]}].map(m=>(
                <div key={m.to} style={{background:'var(--surface2)',border:'1px solid var(--border)',borderRadius:10,padding:12,marginBottom:8}}>
                  ...
                  <div style={{display:'flex',gap:6}}>{m.tags.map(t=><span key={t.label} style={{fontSize:10,padding:'2px 8px',borderRadius:4,color:t.color,background:t.bg}}>{t.label}</span>)}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className={`view${view==='schedule'?' active':''}`}>
            <div className="card">
              <div style={{fontSize:11,letterSpacing:2,color:'var(--muted)',fontFamily:'var(--font-mono)',textTransform:'uppercase',marginBottom:16}}>📅 Construction Schedule</div>
            ...
            <div className="card">
              <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:16}}>
                ...
                <button className="btn btn-primary btn-sm" onClick={()=>alert('Bid requests sent to 4 roofing contractors!')}>📤 Send Bid Requests</button>
              </div>
              <div style={{background:'rgba(255,169,77,.06)',border:'1px solid rgba(255,169,77,.2)',borderRadius:8,padding:12,marginBottom:16,fontSize:12}}>
                <strong style={{color:'var(--warn)'}}>⚡ AI BID ANALYSIS:</strong> Summit Roofing offers the best value — $2,100 below average with a strong 4.8★ rating and earliest start date.
              </div>
              {BIDS.map(b=>(
                <div key={b.name} style={{display:'grid',gridTemplateColumns:'1fr 80px 80px 60px 90px',gap:8,alignItems:'center',padding:'10px 12px',borderRadius:b.recommended?8:0,fontSize:12,background:b.recommended?'rgba(105,219,124,.05)':'',border:b.recommended?'1px solid rgba(105,219,124,.15)':'',marginBottom:b.recommended?8:0}}>
                  ...
                  <button onClick={()=>{setSelectedBid(b.name);alert(`🎉 ${b.name} awarded!`)}} style={{padding:'4px 10px',background:'var(--ok)',color:'#0d0f0f',border:'none',borderRadius:5,fontSize:10,fontWeight:700,cursor:'pointer',opacity:selectedBid&&selectedBid!==b.name?.4:1}}>{selectedBid===b.name?'✓ AWARDED':'SELECT'}</button>
                </div>
              ))}
            </div>
            ...
            <div className="card">
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:12}}>
                <div><div style={{fontSize:11,color:'var(--muted)',marginBottom:4}}>Date</div><input type="date" defaultValue="2025-06-15"/></div>
                <div><div style={{fontSize:11,color:'var(--muted)',marginBottom:4}}>Weather</div><select><option>☀️ Sunny – 65°F</option><option>⛅ Partly Cloudy</option><option>🌧 Rain – Work Suspended</option></select></div>
              </div>
              <div style={{fontSize:11,color:'var(--muted)',marginBottom:4}}>Site Notes</div>
              <textarea value={logText} onChange={e=>setLogText(e.target.value)} rows={4} style={{width:'100%',resize:'none',lineHeight:1.6,fontSize:13,marginBottom:12}}/>
              <div style={{display:'flex',gap:8,alignItems:'center'}}>
                ...
                {logSaved&&<span style={{fontSize:12,color:'var(--ok)'}}>✓ Saved!</span>}
              </div>
            </div>
          </div>
          ...
          <div className={`view${view==='contractors'?' active':''}`}>
              ...
              {CONTRACTORS.map(con=>(
                <div key={con.id} className="contractor-card">
                  ...
                  <div style={{display:'flex',gap:6}}>
                    <div className="btn-icon" onClick={()=>alert(`Calling ${con.name}...`)}>📞</div>
                    <div className="btn-icon" onClick={()=>composeTo(con.name.split(' ').slice(0,2).join(' '),con.name,con.phone)}>💬</div>
                    <div className="btn-icon" onClick={()=>alert(`Opening email to ${con.email}`)}>✉️</div>
                    <div className="btn-icon" onClick={()=>{setSmsBody(SMS_TEMPLATES.bid);document.getElementById('sms-panel')?.scrollIntoView({behavior:'smooth'})}}>💰</div>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="card" id="sms-panel">
              ...
              <div style={{background:'var(--bg)',border:'1px solid var(--border)',borderRadius:10,padding:16}}>
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:12}}>
                  <div><div style={{fontSize:11,color:'var(--muted)',marginBottom:4}}>To</div><input value={smsTo} onChange={e=>setSmsTo(e.target.value)} style={{width:'100%'}}/></div>
                  <div><div style={{fontSize:11,color:'var(--muted)',marginBottom:4}}>Phone</div><input value={smsPhone} onChange={e=>setSmsPhone(e.target.value)} style={{width:'100%'}}/></div>
                </div>
                <div style={{fontSize:11,color:'var(--muted)',marginBottom:4}}>Message ({smsBody.length} chars)</div>
                <textarea value={smsBody} onChange={e=>setSmsBody(e.target.value)} rows={4} style={{width:'100%',background:'var(--surface2)',resize:'none',fontSize:13,lineHeight:1.6,marginBottom:12}}/>
                <div style={{display:'flex',gap:8,alignItems:'center'}}>
                ...
                {smsSent&&<div style={{marginTop:12,padding:12,background:'rgba(105,219,124,.08)',border:'1px solid rgba(105,219,124,.2)',borderRadius:8,fontSize:13,color:'var(--ok)'}}>✓ Message sent! Awaiting contractor reply.</div>}
              </div>
            </div>
          </div>
          ...
          <div className={`view${view==='reports'?' active':''}`}>
            <div className="card">
              ...
              {[{title:'📋 Work Completed Today',items:["Framing Units 2A & 2B progressed to 75% completion (8 workers, 9 hrs)","Electrical pre-wiring complete in Unit 1A (Mike's Electric – 3 workers)","Lumber delivery received (partial – 240 LF 2x6 short, ETA Friday)","Plumbing DWV inspected and approved in Building B1"]},{title:'📈 Current Project Status',items:['Overall completion: 34% (target was 38% by today)','Project is 8 calendar days behind schedule','Budget utilization: $1.47M of $4.2M (35.0%)']},{title:'⚠ Issues & Delays',items:['FRAMING DELAY: Unit 3B framing 40% complete vs. required 80%. Impact: MEP rough-in pushed 6 days.','MATERIAL SHORTAGE: 240 LF of 2x6 lumber pending delivery','ROOFING BID: Award pending — 2 of 4 bids received. Deadline: tomorrow 5 PM']},{title:'📅 Upcoming Work (Next 7 Days)',items:['Mon Jun 16: Foundation inspection w/ engineer, 9 AM','Tue–Wed: Complete framing Units 3A–3C (accelerated crew)','Wed Jun 18: MEP rough-in begins, Buildings A & B','Fri Jun 20: Lumber resupply + roofing award announcement']}].map(section=>(
                <div key={section.title} style={{padding:'16px 0',borderBottom:'1px solid var(--border)'}}>
                  ...
                  {section.items.map(item=><div key={item} style={{display:'flex',gap:8,fontSize:13,padding:'4px 0'}}><span style={{color:'var(--accent)',flexShrink:0}}>▸</span>{item}</div>)}
                </div>
              ))}
        ...
        <div style={{background:'var(--surface)',borderLeft:'1px solid var(--border)',overflowY:'auto',display:'flex',flexDirection:'column'}}>
          <div style={{padding:20,borderBottom:'1px solid var(--border)'}}>
            ...
            <div style={{fontSize:11,color:'var(--muted)',marginTop:4,fontFamily:'var(--font-mono)'}}>
              {aiRunning ? '🔄 Analyzing project...' : aiDone ? 'Last run: just now' : 'Last run: 6 min ago'}
            </div>
          </div>
          ...
          <div style={{margin:16}}>
            ...
            {aiDone && aiMessage && (
              <div style={{marginTop:8,padding:10,background:'rgba(105,219,124,.08)',border:'1px solid rgba(105,219,124,.2)',borderRadius:8,fontSize:11,color:'var(--ok)'}}>
                {aiMessage}
              </div>
            )}
          </div>
              ...
              <div key={i} className="ai-rec">
                ...
                <div style={{display:'flex',gap:6}}>
                  {!actioned.includes(i)
                    ...
                    : <span style={{fontSize:11,color:'var(--ok)',fontWeight:600}}>✓ Logged</span>
                  }
